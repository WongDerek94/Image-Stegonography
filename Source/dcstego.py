#!/usr/bin/env python3
"""
Simple Steganography application that hides a secret image in a cover image.
Encrypts pixels and filename of secret image before using LSB to hide imgage.
"""
...
def path_leaf(path):
    """Accepts the file path of the secret image path to return either the filename of the cover image or directory

    Args:
        path (str): File path of secret image
    Returns:
        tail or ntpath.basename(head): filename of the cover image or directory of cover image
    """
    head, tail = ntpath.split(path)
    return tail or ntpath.basename(head)

def create_stego_image(cover_img_path, secret_img_path, output_path, password):
    """Creates a stego image after validating inputs and encryption

    Args:
        cover_img_path (str): path of cover image
        secret_img_path (str): path of secret image
        output_path (str): path to output stego image
        password (str): password used to encrypt 
    Returns:
        void
    """
...
def extract_secret_image(stego_img, password):
    """Restores secret image from stego image after decryption with password

    Args:
        stego_img (str): path of stego image
...
def factor_passwd(passwd):
    """Validates that the password is factored into a key of length 8, 16, or 24 bytes.

    Args:
        passwd (str): password to be used
    Returns:
        passwd (str): formatted password to a key of length 8, 16, or 24 bytes
    """
    length = len(passwd)
    if length > 24:
...
def check_extension(filename):
    """Check that the image file extension is a .bmp type

    Args:
        filename (str): cover, secret and output names to be validated
    Returns:
        void
    """
